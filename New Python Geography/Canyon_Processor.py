...
def erosion_thermal( amatrix, its = 8, minimum = 5 ):
    ...
    for z in range( 0, its ):
        ...
        print "Thermal Iteration:", z
        imsave("canned_progress.png", amatrix )
...
if(__name__ == '__main__'):
    ...
    a = misc.imread("cave.png")
    print type(a),a.shape,a.dtype

    # excellent time testing tool
    #a = resize(a,(32,32,4))

    b = zeros((a.shape[0],a.shape[1]))
    for x in range(0,a.shape[0]):
        for y in range(0,a.shape[1]):
           b[x][y] = float(a[x][y][0])+float(a[x][y][1])+float(a[x][y][2])

    b = erosion_water(b,30)
    imsave("cave1.png",b)
    b = erosion_water(b,30)
    imsave("cave2.png",b)
    b = erosion_water(b,30)
    ...
    #should take 5.8 hours.
    #started it at 3:16
    #therefore, it aught to finish around 9am
