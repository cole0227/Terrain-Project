...
//
//need mersenne twister to generate the new random.
//also need a matrix of pointers leading to points in 3-space.
// to generate fractal terrain by random midpoint displacement,
// but with non-static major gridlines and variations in both z
// and up to the width of one NEW grid tile in any direction.
...
void Mersenne()
{
	...
	for (int i = 1; i < max;i++)
	{
		int j = extractNumber()%1000000;
		if (i%(max/10)==0) cout << (i/(max/100)) << "%\n";
		if (j<100000) k1++;
		else if (j<500000) k2++;
		else if (j<900000) k3++;
		else k4++;
