...
def generateTerrain():
    ...
    mainMapSize = int( mainMap.size ** ( 0.5 ) )
    scipy.misc.imsave('region_overview.png', mainMap)

    tileDetail = 32
    tileSize = tileDetail * 2

    heightMap = zeros(( mainMapSize * tileSize, mainMapSize * tileSize ))
    ...
    numClouds = 3
    cloud = zeros(( numClouds, tileSize, tileSize ))
    current = 0
    for x in range( 0, numClouds ):
        print '\r',int(100.0*x/numClouds),'%',
        sys.stdout.flush()
        cloud[x] = Matrix_Tiny( clouds.clouds( tileDetail ) )
        save( 'cloud%d.npy' % x, cloud[x] )
        #cloud[x] = load('cloud%d.npy' % x)
    
    print '\rClouds:',(time.clock()-wall),'s'
    wall = time.clock()
    
    for x in range( 0, mainMapSize ):
        print '\r',int(100.0*x/mainMapSize),'%',
        sys.stdout.flush()
        for y in range( 0, mainMapSize ):
                tempMatrix = cloud[current] * ( mainMap[x][y]/5 + 2 ) ** 2
                if ( mainMap[x][y] > 0 ):
                    tempMatrix += 0
                elif (mainMap[x][y] < 0 ):
                    tempMatrix -= 0
                tempMatrix += 10 * mainMap[x][y]
                Matrix_Merge( heightMap, tempMatrix, x * tileSize, y * tileSize )
                if( y % 3 == 0 ):
                    cloud[current] = cloud[current][:, ::-1]                    
                elif( x % 3 == 0 ):
                    cloud[current] = cloud[current][::-1, :]                    
                cloud[current] = rot90(cloud[current])
                current = ( current + 1 ) % numClouds
    
    print '\rMain Loop:',(time.clock()-wall),'s'
    wall = time.clock()

    scipy.misc.imsave('regionHeight.png', heightMap)
    print Stats_Range(heightMap)
    #heightMap = Gaussian_Blur( heightMap, 4 )
    #heightMap = Matrix_Tiny( heightMap )
    #scipy.misc.imsave('regionHeight2.png', heightMap )
    print 'Final Leg:',(time.clock()-wall),'s'


#main( 512, 3600, 2500, 20, 30000)
#main( 256, 900, 500, 5, 7000)
#main( 128, 120, 160, 1, 2000)
#main( 64, 0, 20, 1, 330)
    
wall = time.clock()

main( 128, 120, 160, 0, 0)
generateTerrain()
